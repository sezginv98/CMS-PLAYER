...
app.whenReady().then(() => {
  ...
  fs.readFile(layoutPath, 'utf8', (err, data) => {
    ...
    watchFile('layout.json', 'layout-data');
    watchFile('scroll_text.json', 'scroll-text-data');
  });
});
...
ipcMain.on('get-layout', (event) => {
  fs.readFile(path.join(__dirname, 'layout.json'), 'utf8', (err, data) => {
    if (err) {
      console.error('layout.json okunamadı:', err);
...
ipcMain.on('get-scroll-text', (event) => {
  fs.readFile(path.join(__dirname, 'scroll_text.json'), 'utf8', (err, data) => {
    if (err) {
      event.reply('scroll-text-data', null);
    } else {
      event.reply('scroll-text-data', JSON.parse(data));
    }
  });
});
